...
- [ ] `GET /shipment-tracker/shipment` shows the shipment just created, with the same status data.
...
- [ ] Right after creating a shipment, call `GET /shipment-tracker/shipment` again immediately — confirm `lastCheckedAt` does **not** advance (still within the 30-minute TTL, should serve cached data, no new outbound scrape).
- [ ] Temporarily lower `shipment-tracker.tracking-ttl-minutes` in `application.yaml` (or override via env var) to something short like 1 minute, wait past it, call `GET /shipment-tracker/shipment` again, and confirm `lastCheckedAt` advances (a real re-scrape happened).
- [ ] For an already-`delivered` shipment, confirm repeated `GET /shipment-tracker/shipment` calls never advance `lastCheckedAt` — delivered shipments should never re-scrape.
...
- [ ] If several shipments are stale at once, confirm `GET /shipment-tracker/shipment` still returns successfully even though it's scraping each one serially (just slower, not broken).
